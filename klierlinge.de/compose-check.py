...
container_hashes = subprocess.check_output(["ssh", "www.klierlinge.de", "docker", "ps", "-q"], text=True)
...
for container_hash in container_hashes.strip().split("\n"):
    labels_json = subprocess.check_output(
        ["ssh", "www.klierlinge.de", "docker", "inspect", container_hash, "-f", "'{{json .Config.Labels}}'"],
        text=True,
    )
...
hashes = subprocess.check_output(
    ["ssh", "www.klierlinge.de", "docker", "compose", "-f", "/docker/docker-compose.yaml", "config", "--hash=\\*"], text=True
).strip()
